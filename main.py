...
patient_list = patient_info['检查流水号'].tolist()

# 复制fsaverage_sym和cvs_avg35_inMNI152到指定目录
nt.copy_temp_sub(paths.temp_sub, paths.sym_sub)

# 读取mask列表
mask_list = [os.path.join(paths.SUBJECTS_DIR,i,'seg_mask.nii.gz') for i in patient_list]

# 批量提取所有患者mask在皮质下的体积
# 将mask转换到cvs152空间
mask_cvs152_list, failed_list = nt.convert_mask_to_cvs152(mask_list, os.path.join(paths.temp_sub, 'mri/orig.mgz'))
